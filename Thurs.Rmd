...
Thursday <- day %>% filter(weekday==4) %>% select(-c(casual,registered, instant, dteday))
```

Review Data by Summaries and Plots
```{r, , warning = FALSE}
#Rentals by Temperature
a <- ggplot(Thursday, aes(temp, cnt))
...
```{r, , warning = FALSE}
#Rentals by Season
b <- ggplot(Thursday, aes(x = season, y = cnt))
...
c <- ggplot(Thursday, aes(x = weathersit, y = cnt))
...
```{r}
summary_data <- Thursday %>% select(temp:windspeed)
...
set.seed(1)
trainIndex <- createDataPartition(Thursday$cnt, p = 0.7, list = FALSE)
Thursday.Train <- Thursday[trainIndex, ] 
Thursday.Test <-  Thursday[-trainIndex, ]
```
...
Thursday       <- day %>% filter(weekday==4) %>% select(-c(casual,registered, instant, dteday))
trainIndex   <- createDataPartition(Thursday$cnt, p = 0.7, list = FALSE)
Thursday.Train <- Thursday[trainIndex, ] %>% select(-c(workingday, weekday)) %>% 
  mutate(mnth=as.factor(mnth), season=as.factor(season), weathersit = as.factor(weathersit))
dmy          <- dummyVars(" ~ .", data = Thursday.Train, fullRank = T)
Thursday.Train.trf <- data.frame(predict(dmy, newdata = Thursday.Train)) %>% mutate(y = scale(cnt)) %>% select(-cnt)
fitControl <- trainControl(method = "LOOCV")
model      <- train(y ~., data = Thursday.Train.trf, method = "ctree",
...
```{r, warning = FALSE, error=TRUE}
set.seed(1)
boostFit8 <- gbm(cnt ~., data = Thursday.Train, distribution = "gaussian", n.trees = 100,
                 shrinkage = .1, interaction.depth = 2)
boostPred <- predict(boostFit8, newdata = dplyr::select(Thursday.Test, -cnt), n.trees = 100)
boostRMSE <- sqrt(mean((boostPred-Thursday.Test$cnt)^2))
#Print RMSE
boostRMSE
```
...
set.seed(1)
regFit <- lm(cnt ~ temp + hum + windspeed, data=Thursday.Train)

regPred <- predict(regFit, newdata = dplyr::select(Thursday.Test, -cnt))
```
