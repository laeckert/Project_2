...
Friday <- day %>% filter(weekday==5) %>% select(-c(casual,registered, instant, dteday))
```

Review Data by Summaries and Plots
```{r, , warning = FALSE}
#Rentals by Temperature
a <- ggplot(Friday, aes(temp, cnt))
...
```{r, , warning = FALSE}
#Rentals by Season
b <- ggplot(Friday, aes(x = season, y = cnt))
...
c <- ggplot(Friday, aes(x = weathersit, y = cnt))
...
```{r}
summary_data <- Friday %>% select(temp:windspeed)
...
set.seed(1)
trainIndex <- createDataPartition(Friday$cnt, p = 0.7, list = FALSE)
Friday.Train <- Friday[trainIndex, ] 
Friday.Test <-  Friday[-trainIndex, ]
```
...
Friday       <- day %>% filter(weekday==5) %>% select(-c(casual,registered, instant, dteday))
trainIndex   <- createDataPartition(Friday$cnt, p = 0.7, list = FALSE)
Friday.Train <- Friday[trainIndex, ] %>% select(-c(workingday, weekday)) %>% 
  mutate(mnth=as.factor(mnth), season=as.factor(season), weathersit = as.factor(weathersit))
dmy          <- dummyVars(" ~ .", data = Friday.Train, fullRank = T)
Friday.Train.trf <- data.frame(predict(dmy, newdata = Friday.Train)) %>% mutate(y = scale(cnt)) %>% select(-cnt)
fitControl <- trainControl(method = "LOOCV")
model      <- train(y ~., data = Friday.Train.trf, method = "ctree",
...
```{r, warning = FALSE, error=TRUE}
set.seed(1)
boostFit8 <- gbm(cnt ~., data = Friday.Train, distribution = "gaussian", n.trees = 100,
                 shrinkage = .1, interaction.depth = 2)
boostPred <- predict(boostFit8, newdata = dplyr::select(Friday.Test, -cnt), n.trees = 100)
boostRMSE <- sqrt(mean((boostPred-Friday.Test$cnt)^2))
#Print RMSE
boostRMSE
```
...
set.seed(1)
regFit <- lm(cnt ~ temp + hum + windspeed, data=Friday.Train)

regPred <- predict(regFit, newdata = dplyr::select(Friday.Test, -cnt))
```
