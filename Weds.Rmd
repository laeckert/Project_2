...
Wednesday <- day %>% filter(weekday==3) %>% select(-c(casual,registered, instant, dteday))
```

Review Data by Summaries and Plots
```{r, , warning = FALSE}
#Rentals by Temperature
a <- ggplot(Wednesday, aes(temp, cnt))
...
```{r, , warning = FALSE}
#Rentals by Season
b <- ggplot(Wednesday, aes(x = season, y = cnt))
...
c <- ggplot(Wednesday, aes(x = weathersit, y = cnt))
...
```{r}
summary_data <- Wednesday %>% select(temp:windspeed)
...
set.seed(1)
trainIndex <- createDataPartition(Wednesday$cnt, p = 0.7, list = FALSE)
Wednesday.Train <- Wednesday[trainIndex, ] 
Wednesday.Test <-  Wednesday[-trainIndex, ]
```
...
Wednesday       <- day %>% filter(weekday==3) %>% select(-c(casual,registered, instant, dteday))
trainIndex   <- createDataPartition(Wednesday$cnt, p = 0.7, list = FALSE)
Wednesday.Train <- Wednesday[trainIndex, ] %>% select(-c(workingday, weekday)) %>% 
  mutate(mnth=as.factor(mnth), season=as.factor(season), weathersit = as.factor(weathersit))
dmy          <- dummyVars(" ~ .", data = Wednesday.Train, fullRank = T)
Wednesday.Train.trf <- data.frame(predict(dmy, newdata = Wednesday.Train)) %>% mutate(y = scale(cnt)) %>% select(-cnt)
fitControl <- trainControl(method = "LOOCV")
model      <- train(y ~., data = Wednesday.Train.trf, method = "ctree",
...
```{r, warning = FALSE, error=TRUE}
set.seed(1)
boostFit8 <- gbm(cnt ~., data = Wednesday.Train, distribution = "gaussian", n.trees = 100,
                 shrinkage = .1, interaction.depth = 2)
boostPred <- predict(boostFit8, newdata = dplyr::select(Wednesday.Test, -cnt), n.trees = 100)
boostRMSE <- sqrt(mean((boostPred-Wednesday.Test$cnt)^2))
#Print RMSE
boostRMSE
```
...
regPred <- predict(regFit, newdata = dplyr::select(Wednesday.Test, -cnt))
```
