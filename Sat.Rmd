...
Saturday <- day %>% filter(weekday==6) %>% select(-c(casual,registered, instant, dteday))
```

Review Data by Summaries and Plots
```{r, , warning = FALSE}
#Rentals by Temperature
a <- ggplot(Saturday, aes(temp, cnt))
...
```{r, , warning = FALSE}
#Rentals by Season
b <- ggplot(Saturday, aes(x = season, y = cnt))
...
c <- ggplot(Saturday, aes(x = weathersit, y = cnt))
...
```{r}
summary_data <- Saturday %>% select(temp:windspeed)
...
set.seed(1)
trainIndex <- createDataPartition(Saturday$cnt, p = 0.7, list = FALSE)
Saturday.Train <- Saturday[trainIndex, ] 
Saturday.Test <-  Saturday[-trainIndex, ]
```
...
Saturday       <- day %>% filter(weekday==6) %>% select(-c(casual,registered, instant, dteday))
trainIndex   <- createDataPartition(Saturday$cnt, p = 0.7, list = FALSE)
Saturday.Train <- Saturday[trainIndex, ] %>% select(-c(workingday, weekday)) %>% 
  mutate(mnth=as.factor(mnth), season=as.factor(season), weathersit = as.factor(weathersit))
dmy          <- dummyVars(" ~ .", data = Saturday.Train, fullRank = T)
Saturday.Train.trf <- data.frame(predict(dmy, newdata = Saturday.Train)) %>% mutate(y = scale(cnt)) %>% select(-cnt)
fitControl <- trainControl(method = "LOOCV")
model      <- train(y ~., data = Saturday.Train.trf, method = "ctree",
...
```{r, warning = FALSE, error=TRUE}
set.seed(1)
boostFit8 <- gbm(cnt ~., data = Saturday.Train, distribution = "gaussian", n.trees = 100,
                 shrinkage = .1, interaction.depth = 2)
boostPred <- predict(boostFit8, newdata = dplyr::select(Saturday.Test, -cnt), n.trees = 100)
boostRMSE <- sqrt(mean((boostPred-Saturday.Test$cnt)^2))
#Print RMSE
boostRMSE
```
...
set.seed(1)
regFit <- lm(cnt ~ temp + hum + windspeed, data=Saturday.Train)

regPred <- predict(regFit, newdata = dplyr::select(Saturday.Test, -cnt))
```
